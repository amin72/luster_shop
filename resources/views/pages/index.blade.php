  ...
  <div class="max-w-7xl mx-auto">
    ...
    <section class="rounded-lg bg-white shadow w-full py-4 px-6">
      ...
      <div class="flex justify-between items-center pt-2 pb-8 text-sm xl:text-base flex-wrap">
        @foreach (\App\Models\TopProduct::where('is_active', true)->orderBy('position')->take(6)->get() as $top_product)
        <div class="w-1/2 md:w-1/3 lg:w-1/6 text-center hover:shadow-xl px-1 sm:px-2 py-4">
          <img class="mb-4 rounded h-full w-full mx-auto" src="{{ asset($top_product->product->image) }}" alt="{{ $top_product->product->name }}">
          <span class="text-ceter text-gray-500 mx-auto">{{ $top_product->product->name }}</span>
          <div class="text-left ml-2 text-gray-500 mt-6">{{ $top_product->product->price }} تومان</div>
        </div>
        @endforeach
      </div>
    </section>
